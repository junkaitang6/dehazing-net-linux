...
def preparing_training_data(hazefree_images_dir, hazeeffected_images_dir):
	...
	hazy_pattern = os.path.join(hazeeffected_images_dir, "*.jpg")
	hazy_data = glob.glob(hazy_pattern)

	data_holder = {}

	for h_image in hazy_data:
		#h_image = h_image.split("/")[-1]
		h_image = os.path.basename(h_image)
		id_ = h_image
...
##数据集地址：
		self.train_data, self.validation_data = preparing_training_data('/root/autodl-tmp/Light-DehazeNet-main/data/original_images/GT/', '/root/autodl-tmp/Light-DehazeNet-main/data/training_images/hazy/')
	...
	def __getitem__(self, index):
		...
		hazefree_image = hazefree_image.resize((460,620), Image.ANTIALIAS)
		hazy_image = hazy_image.resize((460,620), Image.ANTIALIAS)
